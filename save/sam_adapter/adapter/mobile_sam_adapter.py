...
class Mobile_sam_adapter(nn.Module):
    def __init__(self, inp_size=1024):
        ...
        self.rebuild = nn.Conv2d(3, 15, kernel_size=3, stride=1, padding=1)
        self.fore = nn.Sequential(
            nn.Conv2d(16, 4, kernel_size=1, stride=1, padding=1),
            nn.Conv2d(4, 1, kernel_size=5, stride=1, padding=1),
        )
    ...
    def forward(self, x):
        ...
        rebuild = self.rebuild(x)
        fusion = torch.cat((rebuild, masks), dim=1)
        mask_fore = self.fore(fusion)
        mask_back = self.back(fusion)
