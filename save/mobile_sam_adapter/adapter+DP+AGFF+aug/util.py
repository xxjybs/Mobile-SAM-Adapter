...
def pred_train(val_loader, model, val_img_names, save_path, gt_folder, inp_size, is_onehot=False):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            pred_fore, pred_back = model(inp)
            pred_mask = (pred_fore > pred_back).float()
            # if isinstance(pred, (list, tuple)):
            #     pred = pred[0]
            # if is_onehot:
            #     pred_mask = pred.argmax(dim=1)[0]
...
def train_one_epoch(
    ...
    for idx, data in enumerate(traindataloader):
        ...
        logit_fore, logit_back = model(input)
        # logit, boundary = model(input)
        loss_fore = criterion_bce(logit_fore, target.float()) + criterion_iou(logit_fore, target.float())
        loss_back = criterion_bce(logit_back, (1-target).float()) + criterion_iou(logit_back, (1-target).float())
        loss = loss_back + loss_fore
        # if is_onehot:
