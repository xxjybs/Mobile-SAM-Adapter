...
class ConvLayer(nn.Module):
    ...
    def forward(self, x, prompt_generator, prompt, block_num):
        # a = prompt_generator.get_prompt(x.permute(0, 2, 3, 1), prompt, block_num, 0)
        for i, blk in enumerate(self.blocks):
            x = prompt_generator.get_prompt(x.permute(0, 2, 3, 1), prompt, block_num, i).permute(0, 3, 1, 2)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, size, prompt_generator, prompt, block_num):
        # x = self.prompt_generator.get_prompt(x_reshape, prompt, i + 1, 1)
        B, _, C = x.size()
        for i, blk in enumerate(self.blocks):
            x = prompt_generator.get_prompt(x.view(B, size, size, C), prompt, block_num, i).view(B, size*size, C)
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x)
            else:
                x = blk(x)
...
class mobilenetv3_block(nn.Module):
    def __init__(self, inp, oup, stride, exp=0.5):
        ...
        self.block1 = MobileBottleneck(oup*16, oup, 3, 1, int(exp*oup))
        self.block2 = MobileBottleneck(inp, oup, 3, stride, int(exp*oup))
        # self.block3 = AGFF(oup, oup).cuda()
        # self.block3 = MobileBottleneck(oup, oup, kernel, stride, int(exp*oup))
    def forward(self, x_reshape, branch_out):
        x_reshape = self.block1(x_reshape)
        branch_out = self.block2(branch_out)
        # out = self.block3(x_reshape, branch_out)
        out = x_reshape + branch_out
        return out
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        branch_dims = [64, 128, 160, 320, 320]
        # self.branch_mobile_v3 = nn.ModuleList()
        # block = MobileBottleneck(embed_dims[0], embed_dims[0]//self.scale_factor, 3, 1, 32)
        # self.branch_mobile_v3.append(block)
        # for i in range(len(branch_dims)-2):
        #     if i >= len(branch_dims)-3:
        #         stride = 1
        #     else:
        #         stride = 2
        #     block = mobilenetv3_block(branch_dims[i]//self.scale_factor, branch_dims[i + 1]//self.scale_factor, stride, 2)
        #     self.branch_mobile_v3.append(block)
    ...
    def forward_features(self, x):
        # x: (N, C, H, W)
        inp = x
        prohandcrafteds = self.prompt_generator.init_handcrafted(inp)

        x = self.patch_embed(x)
        # branch_out = self.branch_mobile_v3[0](x)
        prompt = self.prompt_generator.init_prompt(x.permute(0, 2, 3, 1), prohandcrafteds[0], 1)
        x = self.layers[0](x, self.prompt_generator, prompt, block_num=1)

        # branch_out = self.block2(branch_out, x)
        start_i = 1
        ...
        for i in range(start_i, len(self.layers)):
            ...
            prompt = self.prompt_generator.init_prompt(x_reshape, prohandcrafteds[i], i+1)
